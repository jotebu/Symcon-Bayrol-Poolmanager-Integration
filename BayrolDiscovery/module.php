...
class BayrolDiscovery extends IPSModule
{
    ...
    public function GetConfigurationForm()
    {
        return json_encode([
            'elements' => [
                ...
                ['type' => 'Label', 'caption' => 'Browser: lokale SQLite-Datenbank durchsuchen.'],
    ...
    public function CheckDatabase(): string
    {
        try {
            ...
            $message = 'Datenbank OK. Tabellen: ' . $tables . ', API-Keys: ' . $keys . ', Scans: ' . $scans . ', Tags: ' . $tags . ', Devices: ' . $devices;
    ...
    public function LoadBrowser(): string
    {
        try {
            ...
            return $message;
        } catch (Throwable $e) { return 'Browser-Fehler: ' . $e->getMessage(); }
    }
    ...
    public function LoadApiKeyDetails(): string
    {
        try {
            ...
            $tags = $this->GetTagsForKey($pdo, $key);
    ...
    public function ToggleFavorite(): string
    {
        try {
            ...
            $stmt->execute([':key' => $key]);
            if ($stmt->rowCount() === 0) { return 'API-Key nicht gefunden: ' . $key; }
            $this->UpdateBrowserFormList($this->BuildBrowserRows());
            return 'Favorit umgeschaltet: ' . $key;
        } catch (Throwable $e) { return 'Favorit-Fehler: ' . $e->getMessage(); }
    }
    ...
    private function GetRecentHistory(PDO $pdo, string $key, int $limit): string
    {
        ...
        $stmt->bindValue(':key', $key, PDO::PARAM_STR); $stmt->bindValue(':limit', $limit, PDO::PARAM_INT); $stmt->execute();
        $lines = []; foreach ($stmt->fetchAll() as $row) { $lines[] = $row['observed_at'] . ' | ' . $row['value'] . ' | ' . $row['value_type']; }
        return implode("\n", $lines);
    }

    private function GetTagsForKey(PDO $pdo, string $key): string
    {
        $stmt = $pdo->prepare('SELECT t.name FROM tags t INNER JOIN key_tags kt ON kt.tag_id=t.id WHERE kt.api_key=:key ORDER BY t.name');
        $stmt->execute([':key' => $key]);
    ...
    {
        $known = $this->GetKnownName($key);
        if ($known !== '') {
            $this->AttachTag($pdo, $key, 'verifiziert');
        }
        if (strpos($key, '34.') === 0) { $this->AttachTag($pdo, $key, 'wasserwert'); $this->EnsureDeviceForKey($pdo, 'water', 'Wasserwerte', $key, 'measurement'); }
        if (strpos($key, '13.') === 0) { $this->AttachTag($pdo, $key, 'system'); }
        if (strpos($key, '55.17106.') === 0) { $this->AttachTag($pdo, $key, 'aktor'); $this->AttachTag($pdo, $key, 'filter'); $this->EnsureDeviceForKey($pdo, 'filter_pump', 'Filterpumpe', $key, $this->GetKeySuffix($key)); }
        if (strpos($key, '55.17102.') === 0) { $this->AttachTag($pdo, $key, 'aktor'); $this->AttachTag($pdo, $key, 'licht'); $this->EnsureDeviceForKey($pdo, 'pool_light', 'Poollicht', $key, $this->GetKeySuffix($key)); }
    }

    private function AttachTag(PDO $pdo, string $apiKey, string $tag): void
    {
        $stmt = $pdo->prepare('INSERT OR IGNORE INTO tags(name) VALUES(:name)'); $stmt->execute([':name' => $tag]);
        $tagId = (int)$pdo->query("SELECT id FROM tags WHERE name=" . $pdo->quote($tag))->fetchColumn();
        $rel = $pdo->prepare('INSERT OR IGNORE INTO key_tags(api_key, tag_id) VALUES(:key, :tag)'); $rel->execute([':key' => $apiKey, ':tag' => $tagId]);
    }

    private function EnsureDeviceForKey(PDO $pdo, string $code, string $name, string $apiKey, string $role): void
    {
        $stmt = $pdo->prepare('INSERT OR IGNORE INTO devices(code,name,device_type,first_seen,last_seen) VALUES(:code,:name,:type,:first,:last)');
        $now = date('Y-m-d H:i:s'); $stmt->execute([':code' => $code, ':name' => $name, ':type' => $code, ':first' => $now, ':last' => $now]);
        $pdo->prepare('UPDATE devices SET last_seen=:last WHERE code=:code')->execute([':last' => $now, ':code' => $code]);
        $deviceId = (int)$pdo->query("SELECT id FROM devices WHERE code=" . $pdo->quote($code))->fetchColumn();
        $rel = $pdo->prepare('INSERT OR IGNORE INTO device_keys(device_id, api_key, role) VALUES(:device, :key, :role)'); $rel->execute([':device' => $deviceId, ':key' => $apiKey, ':role' => $role]);
    }

    private function InitializeDatabase(): void
    {
        $dir = $this->GetDatabaseDirectory(); if (!is_dir($dir) && !mkdir($dir, 0775, true) && !is_dir($dir)) { throw new Exception('Datenbankverzeichnis konnte nicht erstellt werden: ' . $dir); }
        $pdo = $this->OpenDatabase(); $pdo->exec('PRAGMA foreign_keys=ON'); $this->CreateSchema($pdo); $this->MigrateSchema($pdo); $this->SeedDefaultTags($pdo);
    }

    private function OpenDatabase(): PDO
    {
        if (!$this->IsSqliteAvailable()) { throw new Exception('PDO SQLite ist nicht verfuegbar.'); }
        $pdo = new PDO('sqlite:' . $this->GetDatabasePath()); $pdo->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION); $pdo->setAttribute(PDO::ATTR_DEFAULT_FETCH_MODE, PDO::FETCH_ASSOC); return $pdo;
    }
    ...
    private function CreateSchema(PDO $pdo): void
    {
        ...
        $pdo->exec('CREATE TABLE IF NOT EXISTS devices (id INTEGER PRIMARY KEY AUTOINCREMENT, code TEXT NOT NULL UNIQUE, name TEXT NOT NULL, device_type TEXT, confidence INTEGER DEFAULT 50, first_seen TEXT NOT NULL, last_seen TEXT NOT NULL, comment TEXT)');
        $pdo->exec('CREATE TABLE IF NOT EXISTS device_keys (device_id INTEGER NOT NULL, api_key TEXT NOT NULL, role TEXT, PRIMARY KEY(device_id, api_key))');
        $pdo->exec('CREATE INDEX IF NOT EXISTS idx_observations_api_key ON observations(api_key)'); $pdo->exec('CREATE INDEX IF NOT EXISTS idx_observations_scan_id ON observations(scan_id)'); $pdo->exec('CREATE INDEX IF NOT EXISTS idx_api_keys_last_seen ON api_keys(last_seen)');
    ...
    {
        $this->AddColumnIfMissing($pdo, 'api_keys', 'display_name', 'TEXT'); $this->AddColumnIfMissing($pdo, 'api_keys', 'is_verified', 'INTEGER DEFAULT 0'); $this->AddColumnIfMissing($pdo, 'api_keys', 'first_firmware', 'TEXT'); $this->AddColumnIfMissing($pdo, 'api_keys', 'last_firmware', 'TEXT'); $this->AddColumnIfMissing($pdo, 'scans', 'firmware', 'TEXT'); $this->AddColumnIfMissing($pdo, 'observations', 'firmware', 'TEXT');
    }

    private function AddColumnIfMissing(PDO $pdo, string $table, string $column, string $definition): void
    {
        $columns = $pdo->query('PRAGMA table_info(' . $table . ')')->fetchAll(); foreach ($columns as $info) { if (($info['name'] ?? '') === $column) { return; } } $pdo->exec('ALTER TABLE ' . $table . ' ADD COLUMN ' . $column . ' ' . $definition);
    }

    private function SeedDefaultTags(PDO $pdo): void
    {
        foreach (['sensor', 'aktor', 'wasserwert', 'temperatur', 'filter', 'licht', 'system', 'experimentell', 'verifiziert'] as $tag) { $pdo->prepare('INSERT OR IGNORE INTO tags(name) VALUES(:name)')->execute([':name' => $tag]); }
    }

    private function ApiGet(array $keys): array
    {
        $host = trim($this->ReadPropertyString('Host')); $port = max(1, min(65535, $this->ReadPropertyInteger('Port'))); $timeout = max(1, $this->ReadPropertyInteger('Timeout')); if ($host === '') { throw new Exception('Host ist leer.'); }
        $url = 'http://' . $host . ':' . $port . '/cgi-bin/webgui.fcgi?sid=' . rawurlencode($this->CreateSid()); $payload = json_encode(['get' => array_values($keys)]); if ($payload === false) { throw new Exception('JSON-Encoding fehlgeschlagen.'); }
        $context = stream_context_create(['http' => ['method' => 'POST', 'header' => "Content-Type: application/json;charset=UTF-8\r\nAccept: application/json\r\n", 'content' => $payload, 'timeout' => $timeout, 'ignore_errors' => true]]); $started = microtime(true); $raw = @file_get_contents($url, false, $context); $durationMs = (int)round((microtime(true) - $started) * 1000); if ($raw === false) { throw new Exception('HTTP request failed.'); }
        $json = json_decode((string)$raw, true); if (!is_array($json)) { throw new Exception('Ungueltige JSON-Antwort.'); } $apiStatus = (int)($json['status']['code'] ?? -1); if ($apiStatus !== 0) { throw new Exception('API Status ' . $apiStatus); } $json['_meta'] = ['duration_ms' => $durationMs, 'requested_keys' => count($keys)]; return $json;
    }

    private function BuildScanKeys(): array
    {
        $keys = []; $suffixes = $this->ParseSuffixes($this->ReadPropertyString('ScanSuffixes')); $max = max(1, min(5000, $this->ReadPropertyInteger('ScanMaxKeys'))); for ($group=max(1,$this->ReadPropertyInteger('ScanGroupStart')); $group<=max($group,$this->ReadPropertyInteger('ScanGroupEnd')); $group++) { for ($object=max(1,$this->ReadPropertyInteger('ScanObjectStart')); $object<=max($object,$this->ReadPropertyInteger('ScanObjectEnd')); $object++) { foreach ($suffixes as $suffix) { $keys[] = $group . '.' . $object . '.' . $suffix; if (count($keys) >= $max) { return $keys; } } } } return $keys;
    }
