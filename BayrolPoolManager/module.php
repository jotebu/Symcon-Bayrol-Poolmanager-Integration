...
class BayrolPoolManager5 extends IPSModule
{
    ...
    public function GetConfigurationForm()
    {
        return json_encode([
            'elements' => [
                ...
                ['type' => 'Label', 'caption' => 'Deaktivierte Discovery-Keys bleiben als Variablen bestehen und werden nur als nicht mehr aktiv gekennzeichnet.']
            ],
            'actions' => [
    ...
    public function ImportDiscovery(): string
    {
        try {
            ...
            foreach ($definitions as $definition) {
                ...
                $deviceCategoryId = $this->EnsureCategory($rootId, 'BPMDevice_' . $this->MakeSafeIdent($deviceCode), $definition['device_name'], 10);
                $positionByDevice[$deviceCode] = ($positionByDevice[$deviceCode] ?? 0) + 10;
                $ident = $this->GetImportedVariableIdent($definition['api_key']);
                $activeIdents[$ident] = true;
                $result = $this->EnsureImportedVariable($deviceCategoryId, $definition, $positionByDevice[$deviceCode]);
                if ($result === 'created') { $created++; }
                elseif ($result === 'renamed') { $renamed++; }
                elseif ($result === 'type_conflict') { $typeConflicts++; }
                else { $reused++; }
            }

            $inactive = $this->MarkInactiveImportedVariables($rootId, $activeIdents);
            $message = 'Discovery-Import angewendet. Aktiviert: ' . count($definitions) . ', neu: ' . $created . ', vorhanden: ' . $reused . ', umbenannt: ' . $renamed . ', Typkonflikte: ' . $typeConflicts . ', deaktiviert vorhanden: ' . $inactive . '.';
            $this->SetValueSafe('DiscoveryImportStatus', $message);
            $this->SetValueSafe('DiscoveryImportedKeys', count($definitions));
            $this->SetValueSafe('DiscoveryInactiveKeys', $inactive);
            return $message;
        } catch (Throwable $e) {
    ...
    public function UpdateValues(): void
    {
        try {
            ...
            $data = $response['data'] ?? [];
            if (!is_array($data)) { throw new Exception('Invalid API data block'); }
    ...
    private function RegisterVariables(): void
    {
        ...
        $this->RegisterVariableInteger('DiscoveryInactiveKeys', 'Discovery Deaktivierte Keys', '', 222);
    }
    ...
    private function CreateProfiles(): void
    {
        ...
        if (!IPS_VariableProfileExists('BPM.FilterOpmode')) { IPS_CreateVariableProfile('BPM.FilterOpmode', VARIABLETYPE_INTEGER); }
    ...
    {
        if (!IPS_VariableProfileExists($name)) { IPS_CreateVariableProfile($name, VARIABLETYPE_FLOAT); }
    ...
    private function UpdateKnownVariables(array $data): void
    {
        ...
        if ($conductivity !== null) { $this->SetValueSafe('Conductivity', $conductivity); }
        $lightStatus = $this->GetIntValue($data, self::API_KEYS['PoolLightStatus']);
        if ($lightStatus !== null) { $this->SetValueSafe('PoolLightActive', $lightStatus === 0); }
        ...
        if ($filterStatus !== null) { $this->SetValueSafe('FilterPumpActive', $filterStatus === 0); }
        $filterOpmode = $this->GetIntValue($data, self::API_KEYS['FilterPumpOpmode']);
        if ($filterOpmode !== null) { $this->SetValueSafe('FilterPumpOpmode', $filterOpmode); }
    ...
    {
        try { return $this->LoadDiscoveryDefinitions(); }
        catch (Throwable $e) { $this->SendDebugMessage('Discovery Import', $e->getMessage()); return []; }
    }
    ...
    private function LoadDiscoveryDefinitions(): array
    {
        ...
        if (!is_file($apiPath)) { throw new Exception('api_keys.csv nicht gefunden: ' . $apiPath); }
        if (!is_file($deviceKeysPath)) { throw new Exception('device_keys.csv nicht gefunden: ' . $deviceKeysPath); }
        ...
        $devices = [];
        foreach ($deviceRows as $row) { $code = trim((string) ($row['code'] ?? '')); if ($code !== '') { $devices[$code] = $row; } }
        $deviceByKey = [];
        $roleByKey = [];
        $sourceByKey = [];
        foreach ($deviceKeyRows as $row) {
            ...
            if ($apiKey === '') { continue; }
            $source = trim((string) ($row['assignment_source'] ?? 'auto'));
            if (!isset($deviceByKey[$apiKey]) || $source === 'manual' || ($sourceByKey[$apiKey] ?? '') !== 'manual') {
                $deviceByKey[$apiKey] = trim((string) ($row['device_code'] ?? ''));
                $roleByKey[$apiKey] = trim((string) ($row['role'] ?? ''));
                $sourceByKey[$apiKey] = $source;
            }
        }
        $definitions = [];
        foreach ($apiRows as $row) {
            if (($row['gateway_import_enabled'] ?? '0') !== '1') { continue; }
            $apiKey = trim((string) ($row['api_key'] ?? ''));
            if ($apiKey === '') { continue; }
            $deviceCode = $deviceByKey[$apiKey] ?? 'unassigned';
            if ($deviceCode === '') { $deviceCode = 'unassigned'; }
            $deviceName = trim((string) ($devices[$deviceCode]['name'] ?? ''));
            if ($deviceName === '') { $deviceName = $deviceCode === 'unassigned' ? 'Nicht zugeordnet' : $deviceCode; }
        ...
        usort($definitions, static function (array $a, array $b): int { $c = strcmp($a['device_code'], $b['device_code']); return $c !== 0 ? $c : strcmp($a['api_key'], $b['api_key']); });
        return $definitions;
    }
    ...
    private function ApplyKnownImportSemantics(array $definition): array
    {
        ...
        switch ($definition['api_key']) {
            ...
            case '55.17102.value':
            case '55.17106.value': $definition['value_type'] = 'string'; break;
        }
        return $definition;
    }
    ...
    private function EnsureImportedVariable(int $parentId, array $definition, int $position): string
    {
        ...
        if ($variableId === false || $variableId <= 0) {
            ...
            if ((int) ($variable['VariableType'] ?? -1) !== $expectedType) { return 'type_conflict'; }
        }
        IPS_SetPosition($variableId, $position);
        $this->SetImportedVariableInfo($variableId, $definition, true);
        $profile = (string) ($definition['profile'] ?? '');
        if ($profile !== '' && IPS_VariableProfileExists($profile)) { IPS_SetVariableCustomProfile($variableId, $profile); }
        $object = IPS_GetObject($variableId);
        $renamed = (($object['ObjectName'] ?? '') !== $definition['variable_name']);
        if ($renamed) { IPS_SetName($variableId, $definition['variable_name']); }
        if ($created) { return 'created'; }
        return $renamed ? 'renamed' : 'reused';
    }

    private function MarkInactiveImportedVariables(int $rootId, array $activeIdents): int
    {
        $inactive = 0;
        foreach (IPS_GetChildrenIDs($rootId) as $deviceCategoryId) {
            foreach (IPS_GetChildrenIDs($deviceCategoryId) as $objectId) {
                $object = IPS_GetObject($objectId);
                $ident = (string) ($object['ObjectIdent'] ?? '');
                if (strpos($ident, 'BPMImport_') !== 0) { continue; }
                if (isset($activeIdents[$ident])) { continue; }
                $info = (string) ($object['ObjectInfo'] ?? '');
                $info = preg_replace('/Importstatus: (aktiv|deaktiviert)/', 'Importstatus: deaktiviert', $info);
                if ($info === null || strpos($info, 'Importstatus: deaktiviert') === false) { $info = trim($info . '; Importstatus: deaktiviert', '; '); }
                IPS_SetInfo($objectId, $info);
                $inactive++;
            }
        }
        return $inactive;
    ...
    private function SetImportedVariableInfo(int $variableId, array $definition, bool $active): void
    {
        IPS_SetInfo($variableId, 'Bayrol Discovery API-Key: ' . $definition['api_key'] . '; Rolle: ' . $definition['role'] . '; Transform: ' . $definition['transform'] . '; Importstatus: ' . ($active ? 'aktiv' : 'deaktiviert'));
    }
    ...
    private function UpdateImportedVariables(array $data, array $definitions): void
    {
        ...
        if ($rootId === false || $rootId <= 0) { return; }
        foreach ($definitions as $definition) {
            if (!array_key_exists($definition['api_key'], $data)) { continue; }
            $deviceCategoryId = @IPS_GetObjectIDByIdent('BPMDevice_' . $this->MakeSafeIdent($definition['device_code']), $rootId);
            if ($deviceCategoryId === false || $deviceCategoryId <= 0) { continue; }
            $variableId = @IPS_GetObjectIDByIdent($this->GetImportedVariableIdent($definition['api_key']), $deviceCategoryId);
            if ($variableId === false || $variableId <= 0) { continue; }
            $raw = $this->CleanString((string) $data[$definition['api_key']]);
            if (($definition['transform'] ?? 'generic') === 'active_when_zero') {
                if (is_numeric($raw)) { SetValue($variableId, ((int) $raw) === 0); }
                continue;
            }
            $variable = IPS_GetVariable($variableId);
            $type = (int) ($variable['VariableType'] ?? VARIABLETYPE_STRING);
            if ($type === VARIABLETYPE_BOOLEAN) { if (is_numeric($raw)) { SetValue($variableId, ((int) $raw) !== 0); } }
            elseif ($type === VARIABLETYPE_INTEGER) { $n = str_replace(',', '.', $raw); if (is_numeric($n)) { SetValue($variableId, (int) ((float) $n)); } }
            elseif ($type === VARIABLETYPE_FLOAT) { $n = str_replace(',', '.', $raw); if (is_numeric($n)) { SetValue($variableId, (float) $n); } }
            else { SetValue($variableId, $raw); }
        }
    }
    ...
    private function EnsureCategory(int $parentId, string $ident, string $name, int $position): int
    {
        ...
        if ($categoryId === false || $categoryId <= 0) { $categoryId = IPS_CreateCategory(); IPS_SetParent($categoryId, $parentId); IPS_SetIdent($categoryId, $ident); }
    ...
    private function ReadCsvAssocFile(string $path): array
    {
        ...
        if (!is_array($header)) { flock($handle, LOCK_UN); fclose($handle); return []; }
        $rows = [];
    ...
    {
        if ($valueType === 'float') { return VARIABLETYPE_FLOAT; }
        if ($valueType === 'integer') { return VARIABLETYPE_INTEGER; }
        if ($valueType === 'boolean-candidate') { return VARIABLETYPE_BOOLEAN; }
        return VARIABLETYPE_STRING;
    }

    private function GetImportedVariableIdent(string $apiKey): string { return 'BPMImport_' . substr(sha1($apiKey), 0, 20); }
    private function MakeSafeIdent(string $value): string { $safe = preg_replace('/[^A-Za-z0-9_]/', '_', $value); return $safe !== null && $safe !== '' ? $safe : 'unknown'; }
    private function GetDiscoveryStorageDirectory(): string { return rtrim(IPS_GetKernelDir(), DIRECTORY_SEPARATOR) . DIRECTORY_SEPARATOR . 'user' . DIRECTORY_SEPARATOR . 'BayrolDiscovery'; }
    ...
    private function ApiGet(array $keys): array
    {
        ...
        if ($httpCode !== 0 && ($httpCode < 200 || $httpCode >= 300)) { throw new Exception('HTTP error ' . $httpCode); }
        $json = json_decode((string) $raw, true);
        if (!is_array($json)) { throw new Exception('Invalid JSON response'); }
        $apiStatus = (int) ($json['status']['code'] ?? -1);
        if ($apiStatus !== 0) { throw new Exception('API status code ' . $apiStatus); }
        $json['_meta'] = ['duration_ms' => $durationMs, 'http_code' => $httpCode, 'requested_keys' => count($keys)];
        return $json;
    }

    private function ExtractHttpCode(array $headers): int { if (isset($headers[0]) && preg_match('/HTTP\/\S+\s+(\d+)/', $headers[0], $match)) { return (int) $match[1]; } return 0; }
    private function CreateSid(): string { return 'SYMBAYROL' . substr(strtoupper(md5((string) microtime(true) . mt_rand())), 0, 23); }
    private function UpdateTimer(): void { $interval = $this->ReadPropertyInteger('UpdateInterval'); if ($interval < 5) { $interval = 0; } $this->SetTimerInterval(self::TIMER_UPDATE, $interval * 1000); }
    private function SetFloatFromKey(string $ident, array $data, string $key): void { if (!array_key_exists($key, $data)) { return; } $value = str_replace(',', '.', $this->CleanString((string) $data[$key])); if (is_numeric($value)) { $this->SetValueSafe($ident, (float) $value); } }
    private function SetIntegerFromKey(string $ident, array $data, string $key): void { $value = $this->GetIntValue($data, $key); if ($value !== null) { $this->SetValueSafe($ident, $value); } }
    private function GetIntValue(array $data, string $key): ?int { if (!array_key_exists($key, $data)) { return null; } $value = $this->CleanString((string) $data[$key]); if ($value === '' || !is_numeric($value)) { return null; } return (int) $value; }
    private function ExtractFirstNumber(string $text): ?float { $normalized = str_replace(',', '.', $text); if (preg_match('/-?[0-9]+(?:\.[0-9]+)?/', $normalized, $match)) { return (float) $match[0]; } return null; }
    ...
    private function ParseFilterDetailedMode(string $text): string { if (stripos($text, 'Eco') !== false) { return 'Eco'; } if (stripos($text, 'Normal') !== false) { return 'Normal'; } if (stripos($text, 'erhoeht') !== false || stripos($text, 'erhöht') !== false || stripos($text, 'High') !== false) { return 'High'; } if ($text === 'Filterpumpe') { return 'Auto/Aus'; } return $text; }
    private function HandleError(string $context, Throwable $e): void { $this->SetValueSafe('ConnectionState', false); $this->SetValueSafe('LastError', $e->getMessage()); $this->SetValueSafe('LastUpdate', date('Y-m-d H:i:s')); $this->SetStatus(self::STATUS_API_ERROR); $this->SendDebugMessage($context . ' error', $e->getMessage()); }
